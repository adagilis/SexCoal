 ...
 *	Modified 2-IX-08 to use vector<> in place of static arrays.
 *
...
//
{
	...
	void operator = ( const Branch& br );	// Equals operator;  has same effect as the copy constructor	
...
// Constructors and destructor:
	...
	ARGNode(int i);
	ARGNode(int nNode, shared_ptr<Chromosome> chr);		// Constructor for a terminal node
	ARGNode(int nNode, shared_ptr<Chromosome> chr,
			double t);								// Constructor for a recombination node
	ARGNode(int nNode, shared_ptr<Chromosome> chr1,
...
// Functions that get values:
	int getNodeNumber();							// Get the identifier number of this node
	double getTime();									// Get the time (= age) of this node
	Context getContext();							// Get the context for this node
	int getNAncestors();							// Get the number of ancestral nodes of this node
	ARGNode * getAncestor(UINT i);					// Get pointer to ancestral node i
	int getNDescendants();							// Get the number of descendant nodes from this node
	shared_ptr < ARGNode > getDescendantNode(int i);				// Get pointer to descendant node i
	vector<Segment> getDescendantSegmentVector(int i);	// Get descendant segment vector i
